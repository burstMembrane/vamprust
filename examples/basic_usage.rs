...
fn main() {
    ...
    let example_lib = library_paths.iter()
    ...
    if let Some(first_lib_path) = example_lib {
        ...
        if let Some(library) = host.load_library(first_lib_path) {
            ...
            if let Some(first_plugin) = plugins.first() {
                println!("\nAttempting to instantiate plugin: {}", first_plugin.identifier);
                
                let sample_rate = 44100.0;
                
                if let Some(mut plugin) = library.instantiate_plugin(first_plugin.index, sample_rate) {
