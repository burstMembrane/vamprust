...
fn build_submodule(submodule_path: &Path) {
    ...
    if !output.status.success() {
        panic!("CMake configure failed: {}", String::from_utf8_lossy(&output.stderr));
    }
    ...
    if !output.status.success() {
        panic!("CMake build failed: {}", String::from_utf8_lossy(&output.stderr));
    }
}
...
fn configure_system_build() {
    ...
    let use_pkg_config = pkg_config::Config::new()
        .probe("vamp-hostsdk")
        .is_ok();
